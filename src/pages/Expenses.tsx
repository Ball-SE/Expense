...
export default function Expenses() {
  const { expenses, totalExpense, loading, addTransaction, updateTransaction, deleteTransaction, uploadReceipt } = useTransactions();
  const { categories, addCategory, loading: loadingCats } = useCategories('expense');
  ...
  return (
    <Layout title="Expense Management">
      <div className="mb-6 md:mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-[1.75rem] font-bold text-blue-900 tracking-tight mb-1">การจัดการรายจ่าย</h2>
          <p className="text-slate-500 text-[12px] md:text-sm">บันทึกและติดตามรายจ่ายทั้งหมดของคุณ</p>
        </div>
      </div>

      {/* Header Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 md:mb-12">
        <div className="bg-surface-lowest p-6 rounded-xl border-l-4 border-tertiary shadow-sm">
          <p className="text-xs font-bold tracking-wider text-slate-400 uppercase mb-2">รายจ่ายเดือนนี้</p>
          <div className="flex items-end gap-2">
            <h3 className="text-2xl md:text-3xl font-extrabold text-tertiary tracking-tight">
              {loading ? '...' : formatCurrency(totalExpense)}
            </h3>
            <span className="text-tertiary text-sm font-medium mb-1 flex items-center">
              <span className="material-symbols-outlined text-sm">trending_down</span> ระบบจริง
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 bg-surface-lowest rounded-2xl p-6 md:p-8 shadow-sm order-2 lg:order-1">
          <div className="mb-6 md:mb-8">
            <h4 className="text-lg md:text-xl font-bold text-blue-900 tracking-tight">{editingId ? 'แก้ไขข้อมูลรายจ่าย' : 'ระบุข้อมูลรายจ่ายใหม่'}</h4>
            ...
            onSubmit={async (e) => {
              ...
              try {
                ...
                const payload = {
                  ...
                  category: formData.category || (categories.length > 0 ? categories[0].name : 'รายจ่ายหลัก'),
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-2">
                  <label className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-widest px-1">วันที่จ่ายเงิน</label>
                  <input 
                    className="w-full bg-surface-container border-none rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-tertiary transition-all outline-none" 
                    type="date"
                    required
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-1 items-center bg-surface-container rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-tertiary">
                    <select 
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input 
                    className="w-full bg-surface-container border-none rounded-xl pl-8 pr-4 py-3 text-sm focus:ring-2 focus:ring-tertiary transition-all outline-none" 
                    placeholder="0.00" 
                    type="number"
                    step="0.01"
                    min="0"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1.5">
                ...
                <div className="flex flex-col gap-2">
                  <select 
                    className="w-full bg-surface-container border-none rounded-xl py-3 md:py-3.5 px-4 focus:ring-2 focus:ring-tertiary font-medium appearance-none outline-none text-sm disabled:opacity-50"
                  ...
                  >
                    ...
                    <option value="add_new" className="font-bold text-tertiary">+ เพิ่มหมวดหมู่ใหม่...</option>
                  </select>
                  {isAddingCategory && (
                    <div className="flex gap-2 items-center bg-surface-container-high p-2 rounded-xl mt-1 animate-in fade-in slide-in-from-top-2 duration-200">
                      <input 
                        type="text" 
                        className="flex-1 bg-surface-lowest border-none rounded-lg py-2.5 px-3 focus:ring-2 focus:ring-tertiary text-sm shadow-inner"
                        placeholder="พิมพ์..."
                      ...
                      <button 
                        type="button"
                        className="bg-tertiary text-white px-4 py-2.5 rounded-lg text-xs font-bold shadow-sm"
                        onClick={async () => {
                          if (!newCategoryName.trim()) return;
                          try {
                            const newCat = await addCategory(newCategoryName.trim(), 'expense');
              ...
              <div className="space-y-1.5">
                ...
                <input 
                  className="w-full bg-surface-container border-none rounded-xl px-4 py-3.5 text-sm focus:ring-2 focus:ring-tertiary transition-all outline-none" 
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-3">
                <input 
                  ...
                  className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-tertiary/10 file:text-tertiary hover:file:bg-tertiary/20 cursor-pointer"
                />
                {(receiptPreview || formData.image_url) && (
                  <div className="relative w-24 h-24 rounded-xl overflow-hidden border-2 border-tertiary/20 shadow-sm">
            ...
            <div className="space-y-3">
              <label className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-widest px-1">รูปแบบการจ่ายเงิน</label>
              <div className="flex gap-4">
                <label className="flex-1 cursor-pointer group">
                  ...
                  <div className="p-4 bg-surface-container border-2 border-transparent peer-checked:border-tertiary/20 peer-checked:bg-tertiary/5 rounded-xl flex items-center justify-center gap-2 md:gap-3 transition-all group-hover:bg-surface-container-high">
                    <span className="material-symbols-outlined text-slate-500 group-peer-checked:text-tertiary">payments</span>
                    <span className="font-medium text-xs md:text-sm text-slate-600 group-peer-checked:text-tertiary">เงินสด</span>
                  </div>
                </label>
                <label className="flex-1 cursor-pointer group">
                  ...
                  <div className="p-4 bg-surface-container border-2 border-transparent peer-checked:border-tertiary/20 peer-checked:bg-tertiary/5 rounded-xl flex items-center justify-center gap-2 md:gap-3 transition-all group-hover:bg-surface-container-high">
                    <span className="material-symbols-outlined text-slate-500 group-peer-checked:text-tertiary">account_balance</span>
                    <span className="font-medium text-xs md:text-sm text-slate-600 group-peer-checked:text-tertiary">เงินโอน</span>
                  </div>
                </label>
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <button 
                className={`${editingId ? 'w-2/3' : 'w-full'} py-3 md:py-4 ${isSubmitting ? 'bg-slate-300' : 'bg-gradient-to-br from-tertiary to-tertiary/80'} text-white font-bold rounded-xl shadow-lg shadow-tertiary/20 flex items-center justify-center gap-2 mt-4 hover:opacity-90 active:scale-[0.98] transition-all`} 
                type="submit"
        ...
        <div className="lg:col-span-5 space-y-6 order-1 lg:order-2">
          <div className="bg-tertiary rounded-2xl p-6 md:p-8 text-white relative overflow-hidden h-48 md:h-64 flex flex-col justify-end">
            <div className="absolute inset-0 z-0 opacity-40">
              <img alt="Finance background" className="w-full h-full object-cover grayscale mix-blend-overlay" src="https://lh3.googleusercontent.com/aida-public/AB6AXuAYwWg8FWFyXWSYQpoIe9GsHSHEsNuc2utG_oqDyxmIofvwHn5HaeYuhD6xOsnSN3dxaPfr5npIHyg8cNcZnSBHdu-TxBSAjW7JuhXRknz6HfiV7856tak6Ff6Lxyw2K4Zpsq2O7fuv7-5_4rtoPIBakaMBmWabq7vJJ9XvvRSPRwAPJQan1ZCP_ErpcV8L49HdVJjv5MgyxG-dBKIviY0W71XBD-OcuGY6m9gdDbH0yVe71zb2-skjeBwInY3KRJlwHYzWwSdXWmQr" />
            </div>
            <div className="relative z-10">
              <h4 className="text-xl md:text-2xl font-black mb-2 tracking-tight">ควบคุมรายจ่าย</h4>
              <p className="text-white/80 text-xs md:text-sm leading-relaxed max-w-xs">การบันทึกรายจ่ายที่แม่นยำเพื่อสุขภาพการเงินที่ดี</p>
            </div>
          </div>
                ...
                expenses.slice(0, 3).map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-surface-container-low transition-colors group">
                    <div className="flex items-center gap-3 md:gap-4">
                      <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center bg-tertiary/10 text-tertiary`}>
                    ...
                    <p className="text-sm font-black text-tertiary">-{formatCurrency(item.amount)}</p>
                  </div>
                ))
              )}
            </div>
      ...
      <div className="mt-8 md:mt-12 bg-surface-lowest rounded-2xl overflow-hidden shadow-sm">
        <div className="p-6 md:p-8 flex flex-col items-start gap-4 border-b border-surface-container">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between w-full gap-4">
            <h4 className="text-lg md:text-xl font-bold text-blue-900 tracking-tight">ประวัติรายจ่าย</h4>
            <div className="text-left md:text-right">
              <p className="text-[10px] md:text-xs text-slate-500 font-bold uppercase tracking-widest mb-1">ยอดรวมตามตัวกรอง</p>
              <h5 className="text-xl md:text-2xl font-black text-tertiary">-{formatCurrency(filteredTotal)}</h5>
            </div>
          </div>
          ...
          <div className="flex flex-row gap-1 bg-surface-container-low p-1.5 rounded-xl mt-2 w-full md:w-auto overflow-x-auto">
            <button 
              ...
              className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-xs md:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === 'all' ? 'bg-surface-lowest text-tertiary shadow-sm' : 'text-slate-500 hover:text-tertiary'}`}
            >
              ทั้งหมด
            </button>
            <button 
              onClick={() => setActiveTab('cash')}
              className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-xs md:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === 'cash' ? 'bg-surface-lowest text-tertiary shadow-sm' : 'text-slate-500 hover:text-tertiary'}`}
            >
              เงินสด
            </button>
            <button 
              onClick={() => setActiveTab('transfer')}
              className={`flex-1 md:flex-none px-6 py-2.5 rounded-lg text-xs md:text-sm font-semibold transition-all whitespace-nowrap ${activeTab === 'transfer' ? 'bg-surface-lowest text-tertiary shadow-sm' : 'text-slate-500 hover:text-tertiary'}`}
            >
              เงินโอน
            </button>
          </div>
                ...
                filteredExpenses.map((row) => (
                  <tr key={row.id} className="hover:bg-surface-container-low/50 transition-colors">
                    ...
                    <td className="px-6 md:px-8 py-4 md:py-5">
                      <span className={`px-2 md:px-3 py-1 bg-tertiary/10 text-tertiary text-[8px] md:text-[10px] font-bold rounded-full uppercase tracking-tighter`}>{row.payment_method === 'cash' ? 'เงินสด' : 'เงินโอน'}</span>
                    </td>
                    <td className="px-6 md:px-8 py-4 md:py-5 text-xs md:text-sm font-bold text-tertiary">-{formatCurrency(row.amount)}</td>
                    <td className="px-6 md:px-8 py-4 md:py-5 text-right">
                      <div className="flex justify-end gap-1 md:gap-2">
                        <button 
                          ...
                          className="w-7 h-7 md:w-9 md:h-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-tertiary hover:bg-tertiary/5 transition-all"
                        >
                        ...
                        <button 
                          ...
                          className="w-7 h-7 md:w-9 md:h-9 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all"
                        >
